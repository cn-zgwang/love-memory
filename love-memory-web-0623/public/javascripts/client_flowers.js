...
client['updateWish'] = updateWish;
// connect method
function connect(callback){
	// ip and port, you should change the ip address by yourself.
	socket = io.connect("http://47.97.104.77:8128/flower");
	// socket = io.connect("http://127.0.0.1:8128/flower");
...
*/
function register(phone, type, callback){
	connect(callback);
...
function updateWish(plateform, orderId, phone, setPhone, head, content, name,
	flowerShop, callback){
	connect(callback);
	...
	obj.flowerShop = flowerShop;
	obj.orderId = orderId;
	socket.emit("updateWish", obj);

	socket.on('updateWishSucceed', function(object){
		callback.successCallback(object);
	});
	socket.on('updateWishFailed', function(object){
		callback.errorCallback(object);
	});
}
