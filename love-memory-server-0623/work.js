...
var message = require("./lib/service/sendMessageAli");
var sSocket = null;
...
function work(io){
	...
	sSocket && (function() {
		sSocket.of("/flower").on('connection', function(cSocket) {
			...
			// create a wish.
			cSocket.on('getOrderId', function(orderId){
				clearTimeout(authTimer);
				// create a wish
				service.readOrderIds(orderId, function(err, res){
					console.log(res);
					var socketConnect = sSocket.of('/flower').connected[cSocket.id];
					if (res >= -1) {				
						socketConnect && socketConnect.emit('getOrderIdSucceed', res);
						console.log(res);
					} else {
						socketConnect && socketConnect.emit('getOrderIdFailed', err);
					 	console.log(err);
					}
				});
			});
			// update wishes
			cSocket.on('updateWish', function(obj){
				clearTimeout(authTimer);
				// console.log(obj);
				// update a wish
				service.updateWish(obj, function(err, res){
					var socketConnect = sSocket.of('/flower').connected[cSocket.id];
					if (res) {				
						socketConnect && socketConnect.emit('updateWishSucceed', res);
						console.log(res);
					} else {
						socketConnect && socketConnect.emit('updateWishFailed', err);
					 	console.log(err);
					}
				});
			});
			// delete a wish.
			cSocket.on('deleteWish', function(obj){
				clearTimeout(authTimer);
				// create a wish
				service.deleteWish(obj, function(err, res){
					var socketConnect = sSocket.of('/flower').connected[cSocket.id];
					if (res) {				
						socketConnect && socketConnect.emit('deleteWishSucceed', res);
						console.log(res);
					} else {
						socketConnect && socketConnect.emit('deleteWishFailed', err);
					 	console.log(err);
					}
				});
			});
